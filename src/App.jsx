...
const RegulaminView = ({ setCurrentView, lang }) => (
  <div className="min-h-screen bg-white dark:bg-black p-6 md:p-16 font-sans transition-colors duration-500">
    <div className="max-w-3xl mx-auto text-slate-700 dark:text-slate-300">
      ...
      <LegalH>3. {lang === 'EN' ? 'Prompt disclaimer' : 'Zastrzezenia dot. promptow'}</LegalH>
      <LegalP>{lang === 'EN' ? 'The prompt creators on AI Flow Academy generate ready-to-use text prompts intended for use in external AI image generation tools (e.g. Midjourney, Ideogram, NanoBanana and others). AI Flow Academy is not responsible for the final visual result, as the interpretation of the prompt depends solely on the external tool used. The user has the right to edit the prompt after copying it. Results may vary depending on the tool, its version, and settings used.' : 'Kreatory promptow AI Flow Academy tworza gotowe prompty tekstowe przeznaczone do uzycia w zewnetrznych narzędziach generujacych obrazy AI (np. Midjourney, Ideogram, NanoBanana i innych). AI Flow Academy nie ponosi odpowiedzialnosci za ostateczny efekt wizualny uzyskany przy uzyciu promptu, poniewaz interpretacja tekstu zalezy wylacznie od zewnetrznego narzedzia. Uzytkownik ma prawo do dowolnej edycji promptu po jego skopiowaniu. Efekty moga sie roznic w zaleznosci od narzedzia, jego wersji i ustawien.'}</LegalP>
      <LegalP>{lang === 'EN' ? '⚠️ AI Flow Academy prompt creators are not intended for generating 18+ or adult content. Users are solely responsible for how they use the generated prompts. The platform reserves the right to suspend accounts that violate this policy.' : '⚠️ Kreatory AI Flow Academy nie sa przeznaczone do generowania tresci 18+ ani materialow dla doroslych. Uzytkownik ponosi pelna odpowiedzialnosc za sposob wykorzystania wygenerowanych promptow. Platforma zastrzega sobie prawo do zawieszenia kont naruszajacych te zasade.'}</LegalP>
