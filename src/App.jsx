    ...
    <div className="font-sans flex flex-col pb-20">
        ...
        <div className="max-w-4xl mx-auto bg-black rounded-2xl overflow-hidden shadow-2xl relative aspect-video border border-slate-200 dark:border-[#1A1A1A]">
          ...
          {isVideoActive && (
            <div className="absolute inset-0 w-full h-full bg-black">
              {/* ZMIANA: Adres youtube-nocookie.com oraz allow="autoplay" */}
              <iframe 
                width="100%" 
                height="100%" 
      ...
      <div ref={pricingRef} className="pt-16 pb-20 px-4 bg-white dark:bg-[#050505] transition-colors duration-500 font-sans">
        <div className="max-w-5xl mx-auto flex flex-col items-center">
          <h2 className="text-3xl font-bold mb-12 text-slate-900 dark:text-[#E5E0D8] uppercase tracking-tighter font-sans">Subskrypcja Premium</h2>
          <div className="grid md:grid-cols-2 gap-8 w-full max-w-4xl">
            <div className="bg-slate-50 dark:bg-[#0A0A0A] rounded-3xl p-8 border border-slate-200 dark:border-[#1A1A1A] flex flex-col">
              <h3 className="text-xl font-bold mb-4 dark:text-white uppercase font-sans">Abonament Miesięczny</h3>
              <div className="text-4xl font-extrabold mb-6 dark:text-white font-sans">199 PLN <span className="text-sm text-slate-500 font-normal">/ msc</span></div>
              <ul className="space-y-4 mb-8 flex-grow">
                {['Baza VOD', 'Live Coaching 3x/tydz', 'Prompt Builder Pro', 'Zamknięta Grupa'].map(f => (
                  <li key={f} className="flex items-center gap-2 text-sm dark:text-slate-300 font-sans"><Check className="w-4 h-4 text-emerald-500" /> {f}</li>
                ))}
              </ul>
            ...
            <div className="bg-slate-900 dark:bg-black rounded-3xl p-8 border border-amber-500/30 shadow-2xl relative scale-105 flex flex-col">
              ...
              <h3 className="text-xl font-bold mb-4 text-white uppercase font-sans">Plan Roczny VIP</h3>
              <div className="text-4xl font-extrabold mb-6 text-white font-sans">150 PLN <span className="text-sm text-slate-400 font-normal">/ msc</span></div>
              <ul className="space-y-4 mb-8 flex-grow">
                {['Wszystko w Pro', 'Gwarancja stałej ceny', 'Bonusy VIP', 'Certyfikat Eksperta'].map(f => (
                  <li key={f} className="flex items-center gap-2 text-sm text-slate-200 font-sans"><Check className="w-4 h-4 text-amber-500" /> {f}</li>
                ))}
              </ul>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center p-4 z-50 font-sans">
          <div className="bg-white dark:bg-[#0A0A0A] rounded-2xl p-8 max-w-md w-full border dark:border-[#1A1A1A] relative shadow-2xl">
            ...
            <h3 className="text-2xl font-bold mb-2 text-center dark:text-[#E5E0D8] uppercase tracking-tighter font-sans">Finalizacja</h3>
            <p className="text-center text-[10px] text-slate-500 uppercase tracking-widest mb-6 font-bold font-sans">Pakiet: <span className="text-amber-500">{selectedPlan}</span></p>
            <div className="space-y-4">
              <button className="w-full p-4 border rounded-xl flex items-center gap-4 dark:bg-black dark:border-[#1A1A1A] dark:text-white hover:border-amber-500 transition-colors group text-left">
                ...
                <div><div className="font-bold text-sm">Płatność Automatyczna</div><div className="text-[10px] text-slate-500 font-bold uppercase">BLIK, Karta, Apple Pay</div></div>
              </button>
    ...
    <div className="pb-20 p-2 md:p-8 bg-[#F4EFE6] dark:bg-black transition-colors duration-700 min-h-screen">
      <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-3">
          <div className="mb-10 flex items-center justify-between border-b border-black dark:border-[#222] pb-6 font-sans">
            <h1 className="text-3xl font-serif text-black dark:text-white flex items-center gap-3">
              ...
              PROMPT STUDIO <span className="text-[10px] font-sans bg-amber-500 text-black px-2 py-1 ml-2 font-bold uppercase tracking-widest relative -top-1">Edition Limitée</span>
            </h1>
          </div>
          ...
          <div className={sectionClass}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 font-sans">
              <div><label className={labelClass}>Podmiot</label><select value={subject} onChange={(e) => setSubject(e.target.value)} className={inputClass}>
                  ...
                  <option value="1 trans woman, beautiful">1 Trans Kobieta</option>
              ...
              <div><label className={labelClass}>Sylwetka</label><select value={bodyType} onChange={(e) => setBodyType(e.target.value)} className={inputClass}>
                  ...
                  <option value="thick body, chubby">Krągła</option>
              </select></div>
              <div><label className={labelClass}>Biust</label><select value={breastSize} onChange={(e) => setBreastSize(e.target.value)} className={inputClass}>
                  ...
                  <option value="pectoral muscles">Męska klatka</option>
              </select></div>
              ...
              <div><label className={labelClass}>Owłosienie</label><select value={bodyHair} onChange={(e) => setBodyHair(e.target.value)} className={inputClass}>
                  ...
                  <option value="hairy body, thick chest hair">Mocne (Bear)</option>
              </select></div>
            </div>
          </div>
          ...
          <div className={sectionClass}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 font-sans">
              <div><label className={labelClass}>Fryzura (Odsłonięta szyja)</label><select value={hairStyle} onChange={(e) => setHairStyle(e.target.value)} className={inputClass}>
                  <option value="elegant updo hair, wedding style, revealing ears and earrings">Upięcie ślubne</option>
                  <option value="high bun hair, sleek look, exposing neck and jewelry">Wysoki kok</option>
                  <option value="tied in a ponytail, exposing neck and ears">Kucyk</option>
              </select></div>
              <div><label className={labelClass}>Kolor</label><select value={hairColor} onChange={(e) => setHairColor(e.target.value)} className={inputClass}>
                  ...
                  <option value="blue hair">Niebieskie</option>
              </select></div>
              <div><label className={labelClass}>Długość</label><select value={hairLength} onChange={(e) => setHairLength(e.target.value)} className={inputClass}>
                  ...
                  <option value="bald">Łysy/a</option>
              </select></div>
              <div><label className={labelClass}>Twarz detale</label><select value={faceSelect} onChange={(e) => setFaceSelect(e.target.value)} className={inputClass}>
                  <option value="detailed symmetrical face">Klasyczna</option>
                  <option value="cute face, freckles">Piegi</option>
                  <option value="handsome face, stubble">Zarost</option>
              </select></div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className={sectionClass}>
              <h2 className={headerClass}><Gem className="w-4 h-4" /> V. Ornement & Bijoux</h2>
              <div className="grid grid-cols-2 gap-4 font-sans">
                <div><label className={labelClass}>Kolczyki</label><select value={jewelry} onChange={(e) => setJewelry(e.target.value)} className={inputClass}>
                  <option value="wearing luxury pearl drop earrings">Perły</option>
                  <option value="wearing large gold hoop earrings">Złote Koła</option>
                  <option value="extravagant chandelier earrings">Chandelier</option>
                  <option value="multiple silver ear piercings">Zestaw Piercingu</option>
                </select></div>
                <div><label className={labelClass}>Piercing Twarzy</label><select onChange={(e) => setJewelry(prev => prev + ', ' + e.target.value)} className={inputClass}>
                  <option value="">+ Dodaj</option>
                  <option value="septum ring">Septum</option>
                  <option value="lip piercing">Warga</option>
                  <option value="nose stud">Nostril</option>
                </select></div>
              </div>
            </div>

            <div className={sectionClass}>
              <h2 className={headerClass}><Paintbrush className="w-4 h-4" /> VI. Visage Art</h2>
              <div className="grid grid-cols-2 gap-4 font-sans">
                <div><label className={labelClass}>Makijaż</label><select value={makeup} onChange={(e) => setMakeup(e.target.value)} className={inputClass}>
                  <option value="cat eyes, sharp winged eyeliner">Cat Eyes</option>
                  <option value="glamour makeup, red lipstick">Glamour</option>
                  <option value="sultry smokey eyes, heavy makeup">Smokey Eyes</option>
                  <option value="gothic makeup, black lips">Gothic</option>
                </select></div>
                <div><label className={labelClass}>Detale</label><select onChange={(e) => setMakeup(prev => prev + ', ' + e.target.value)} className={inputClass}>
                  <option value="">+ Dodaj</option>
                  <option value="long eyelashes">Rzęsy</option>
                  <option value="glossy lips">Błyszczyk</option>
                  <option value="face glitter">Brokat</option>
                </select></div>
              </div>
            </div>
          </div>
          ...
          <div className={sectionClass}>
            <h2 className={headerClass}><Shirt className="w-4 h-4" /> III. Haute Couture & Footwear</h2>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 font-sans">
              <div><label className={labelClass}>Góra</label><select value={topClothing} onChange={(e) => setTopClothing(e.target.value)} className={inputClass}>
                  ...
                  <option value="bare chest">Nagi tors</option>
              </select></div>
              <div><label className={labelClass}>Dół</label><select value={bottomClothing} onChange={(e) => setBottomClothing(e.target.value)} className={inputClass}>
                  ...
                  <option value="speedos">Slipy</option>
              </select></div>
              <div><label className={labelClass}>Nogi</label><select value={legwear} onChange={(e) => setLegwear(e.target.value)} className={inputClass}>
                  ...
                  <option value="white socks">Skarpetki</option>
              </select></div>
              <div><label className={labelClass}>Obuwie</label><select value={shoes} onChange={(e) => setShoes(e.target.value)} className={inputClass}>
                  <option value="elegant high heels, stilettos">Szpilki</option>
                  <option value="modern running shoes">Sportowe</option>
                  <option value="leather combat boots">Glany</option>
                  <option value="barefoot, detailed toes">Boso</option>
              </select></div>
              <div><label className={labelClass}>Detal butów</label><select value={shoeDetail} onChange={(e) => setShoeDetail(e.target.value)} className={inputClass}>
                  <option value="none">Standard</option>
                  <option value="red soles">Czerwona podeszwa</option>
                  <option value="shiny patent leather">Lakier</option>
                  <option value="neon glowing soles">Neon (Cyber)</option>
              </select></div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-10 font-sans">
            <div className="bg-[#FCFBF9] dark:bg-[#0A0A0A] border border-black dark:border-[#333] p-6 shadow-[8px_8px_0px_#5E626B] dark:shadow-none transition-colors duration-500">
              <h2 className="text-[10px] font-bold uppercase tracking-widest mb-4 border-b border-black dark:border-[#333] pb-2 dark:text-amber-500 font-sans tracking-widest uppercase">Kompilacja Promptu</h2>
              <div className="bg-[#EBECEF] dark:bg-[#121212] p-5 min-h-[220px] text-black dark:text-[#E5E0D8] font-mono text-[11px] leading-relaxed break-words border border-black dark:border-[#222] mb-6 shadow-inner transition-colors duration-500">
                <span className="text-amber-600 dark:text-amber-500 font-bold select-none font-mono">{`> `}</span>{generatePrompt()}
                <span className="animate-pulse text-amber-500 font-bold font-mono">|</span>
              </div>
              <button onClick={handleCopy} className={`w-full py-4 font-bold text-[10px] uppercase tracking-widest border border-black dark:border-amber-500 transition-all font-sans ${copied ? 'bg-emerald-500 text-black shadow-none' : 'bg-black dark:bg-[#1A1A1A] text-amber-500 hover:bg-amber-500 hover:text-black shadow-[4px_4px_0px_black] dark:shadow-none'}`}>
      ...
      <div className="min-h-screen bg-white dark:bg-black transition-colors duration-700 font-sans selection:bg-amber-500 selection:text-black">
        <nav className="bg-slate-900 dark:bg-[#050505] border-b border-slate-800 dark:border-[#1A1A1A] sticky top-0 z-50 h-16 flex items-center px-4 font-sans">
          <div className="max-w-[1400px] mx-auto w-full flex items-center justify-between">
            <div className="flex items-center gap-2 cursor-pointer font-sans" onClick={() => setCurrentView('home')}>
        ...
        <footer className="bg-slate-50 dark:bg-[#050505] py-10 border-t border-slate-200 dark:border-[#111] transition-colors duration-500 font-sans">
           <div className="max-w-[1400px] mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-6 font-sans">
              <p className="text-[9px] text-slate-500 dark:text-slate-600 uppercase tracking-widest font-bold font-sans">© 2026 Damian L. J. - Professional AI Suite</p>
           </div>
        </footer>
      </div>
    </div>
