...
// 3 kreatory: Napisy, Kolorowanki, Koszulki

import React, { useState, useRef, useCallback } from 'react';
import { Copy, Check, Sparkles, ArrowLeft } from 'lucide-react';

// ─── NAWIGACJA (identyczna z App.jsx) ────────────────────────────────────────
function Nav() {
  return (
    <>
...
// ─── PROMPT OUTPUT (wspólny komponent) ───────────────────────────────────────
function PromptOutput({ prompt, onRegenerate, color = '#f59e0b', promptId = 'prompt-output' }) {
  const [copied, setCopied] = useState(false);
  async function handleCopy() {
    try { await navigator.clipboard.writeText(prompt); setCopied(true); setTimeout(() => setCopied(false), 2500); }
    catch { const el = document.getElementById(promptId); if (el) { const r = document.createRange(); r.selectNode(el); window.getSelection().removeAllRanges(); window.getSelection().addRange(r); } }
  }
  return (
    <div className="rounded-2xl p-6 space-y-4" style={{ background: `${color}08`, border: `1px solid ${color}33` }}>
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-[10px] text-white/30 mt-0.5">Wklej do Midjourney, Ideogram lub NanoBanana</p>
        </div>
        <div className="flex gap-2">
          {onRegenerate && (
            <button onClick={onRegenerate} className="flex items-center gap-2 px-4 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all"
              style={{ background: `${color}15`, border: `1px solid ${color}44`, color }}>
              🎲 Nowa losowa
            </button>
          )}
          <button onClick={handleCopy} className="flex items-center gap-2 px-6 py-2.5 rounded-xl font-black text-[11px] uppercase tracking-widest transition-all"
            style={{
              background: copied ? 'rgba(34,197,94,0.15)' : `${color}15`,
              border: `1px solid ${copied ? 'rgba(34,197,94,0.4)' : color + '44'}`,
      ...
      <textarea id={promptId} readOnly value={prompt} onClick={e => e.target.select()}
...
const TEXT_STYLES = [
  ...
  { id: 'redstone',  emoji: '🪨', label: 'Czerwony kamień',    desc: 'czerwony granit, czarny marmur',color: '#ef4444', prompt: 'sculpted from deep crimson red granite stone with rough natural texture, placed on polished black marble surface, dramatic spotlight from above, luxury stone carving, architectural photography' },
  ...
  { id: 'subscribe', emoji: '🔴', label: 'Subskrybuj',         desc: 'czerwony YouTube, biały blask', color: '#dc2626', prompt: 'bold bright red glossy material like YouTube subscribe button, smooth lacquered red surface with white reflections, clean modern design, pure white background, product shot' },
...
const TEXT_BACKGROUNDS = [
  ...
  { id: 'neon',        label: 'Neon / Cyber',      prompt: 'dark cyberpunk background with neon grid lines, deep black with purple and cyan neon glow',     preview: 'linear-gradient(135deg,#0a0a1a,#1a0a2e)' },
];
  ...
  const draw = useCallback(() => {
    ...
    if (portrait) {
      ...
      letters.forEach((letter, i) => {
        const t = n === 1 ? 0.5 : i / (n - 1), x = n === 1 ? W / 2 : startX + t * xSwing, y = startY + i * cellSize, col = LCOLORS[i % LCOLORS.length];
      ...
      letters.forEach((letter, i) => {
        const t = n === 1 ? 0.5 : i / (n - 1), x = startX + i * cellSize, y = n === 1 ? H / 2 : startY + t * ySwing, col = LCOLORS[i % LCOLORS.length];
        ctx.save(); ctx.globalAlpha = 0.13; ctx.fillStyle = col; ctx.beginPath(); ctx.arc(x, y, fontSize * 0.54, 0, Math.PI * 2); ctx.fill(); ctx.restore();
        ctx.save(); ctx.globalAlpha = 0.28; ctx.fillStyle = '#000'; ctx.fillText(letter, x + 1.5, y + 2); ctx.restore();
        ctx.fillStyle = col; ctx.globalAlpha = 1; ctx.fillText(letter, x, y);
      });
...
function buildTextPrompt(text, style, bg, format) {
  const letters = text.trim().toUpperCase().split('');
  const isMulti = letters.length > 1;
  const arrangement = format === 'portrait' ? 'arranged diagonally from upper-left to lower-right, stacked vertically, each letter slightly offset to the right, filling the vertical 9:16 frame' : 'arranged diagonally from upper-left to lower-right along a horizontal axis, each letter slightly lower than the previous, filling the wide 16:9 frame';
  const subject = isMulti ? `the letters ${letters.join(', ')} spelling "${text.toUpperCase()}", ${arrangement}` : `a single large decorative letter "${text.toUpperCase()}" centered and filling the frame`;
...
function TextCreatorView({ onBack }) {
  ...
  function handleGenerate() {
    if (!canGen) return;
    setPrompt(buildTextPrompt(text, selStyle, selBg, format));
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pb-24">
      ...
      <div className="text-center mb-10">
        ...
        <h1 className="text-3xl font-black uppercase tracking-tighter mb-2">Kreator <span style={{ color: '#f59e0b' }}>Napisów</span></h1>
      ...
      <div className="space-y-10">
        ...
        <Step n="1" title="Format i tekst">
          <div className="flex gap-3 mb-6">
            {[{ id: 'portrait', emoji: '📱', label: '9:16 Pionowy', sub: 'TikTok / Reels · max 6 znaków' }, { id: 'landscape', emoji: '🖥️', label: '16:9 Poziomy', sub: 'YouTube / desktop · max 10 znaków' }].map(f => (
          ...
          <div className="flex flex-col sm:flex-row gap-6 items-start">
            ...
            <div className="flex-1 pt-6">
              <input type="text" maxLength={maxChars} placeholder={format === 'portrait' ? 'np. A, Ania, LOVE' : 'np. AIFLOW, SUMMER'} value={text}
        ...
        {prompt && <div ref={promptRef}><PromptOutput prompt={prompt} color="#f59e0b" promptId="text-prompt" /></div>}
      </div>
    </div>
  );
}
...
const COLORING_CATEGORIES = [
  ...
  { id: 'newyear',      emoji: '🎆', label: 'Nowy Rok',            group: 'święta',   prompts: ['fireworks exploding over a city skyline at midnight','people celebrating New Year in a city square with confetti','a champagne toast with fireworks visible through a window','a clock showing midnight with party decorations','Times Square style New Year celebration with crowds','a family watching fireworks from a hilltop','a New Year party table with glasses and countdown clock','couples dancing at an elegant New Year Eve gala'] },
];

const DIFFICULTY = [
  { id: 'tiny',   emoji: '👶', label: 'Maluszki (2-4 lata)',  desc: 'Bardzo grube linie, proste kształty', prompt: 'very simple thick bold black outlines, minimal details, designed for toddlers ages 2-4, large simple shapes, no tiny details, clear uncluttered composition' },
  { id: 'kids',   emoji: '🧒', label: 'Dzieci (4-8 lat)',     desc: 'Grube linie, proste szczegóły',        prompt: 'simple thick black outlines, basic details, designed for young children ages 4-8, clear shapes with some simple inner details, friendly and cute style' },
  { id: 'junior', emoji: '🧑', label: 'Junior (8-12 lat)',    desc: 'Średnie linie, więcej detali',          prompt: 'medium weight black outlines, moderate detail level, designed for children ages 8-12, good balance of simple areas and detailed sections' },
  { id: 'teen',   emoji: '🧑‍🎓', label: 'Młodzież (12-16 lat)', desc: 'Cienkie linie, szczegółowe',          prompt: 'fine detailed black outlines, intricate details, designed for teenagers, complex composition with many elements to color' },
  { id: 'adult',  emoji: '🎨', label: 'Dorośli',              desc: 'Bardzo szczegółowe, zawiłe',           prompt: 'very fine intricate black outlines, highly detailed complex illustration, designed for adults, dense composition with many small details, professional coloring book quality' },
];

const COLORING_GROUPS = [
  { id: 'all',      label: 'Wszystkie' },
  { id: 'dzieci',   label: 'Dla dzieci' },
  { id: 'mlodzież', label: 'Młodzież' },
  { id: 'dorośli',  label: 'Dorośli' },
  { id: 'święta',   label: 'Święta' },
];

function ColoringCreatorView({ onBack }) {
  ...
  const filtered = activeGroup === 'all' ? COLORING_CATEGORIES : COLORING_CATEGORIES.filter(c => c.group === activeGroup);
  const canGen = selCategory && selDifficulty;

  function buildPrompt() {
    const scene = selCategory.prompts[Math.floor(Math.random() * selCategory.prompts.length)];
    return `Black and white coloring book page illustration of ${scene}. ${selDifficulty.prompt}. Pure white background, clean crisp black lines only, no grey shading, no color fills, no gradients, no shadows — only black outlines on white background. Professional coloring book style, print-ready for 8.5x11 inch KDP page. --ar 3:4 --style raw --v 6.1`;
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pb-24">
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-white/40 text-sm">Generuj prompty kolorowanek do Midjourney lub Ideogram — gotowe do druku na Amazon KDP</p>
      ...
      <div className="space-y-10">
        <Step n="1" title="Wybierz kategorię" color="#a78bfa">
          <div className="flex flex-wrap gap-2 mb-4">
            {COLORING_GROUPS.map(g => (
              <button key={g.id} onClick={() => setActiveGroup(g.id)} className="px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all"
                style={{ background: activeGroup === g.id ? '#a78bfa' : 'rgba(167,139,250,0.1)', color: activeGroup === g.id ? '#000' : '#a78bfa', border: `1px solid ${activeGroup === g.id ? '#a78bfa' : 'rgba(167,139,250,0.3)'}` }}>
                {g.label}
              </button>
            ))}
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {filtered.map(cat => (
              <button key={cat.id} onClick={() => setSelCategory(cat)} className="relative p-4 rounded-xl text-left transition-all hover:scale-[1.02]"
                style={{ background: selCategory?.id === cat.id ? 'rgba(167,139,250,0.15)' : 'rgba(255,255,255,0.03)', border: `1px solid ${selCategory?.id === cat.id ? 'rgba(167,139,250,0.5)' : 'rgba(255,255,255,0.08)'}` }}>
                ...
                {selCategory?.id === cat.id && <div className="absolute top-2 right-2 w-4 h-4 rounded-full flex items-center justify-center" style={{ background: '#a78bfa' }}><Check className="w-2.5 h-2.5 text-black" /></div>}
              </button>
            ))}
          </div>
        </Step>
        <Step n="2" title="Poziom trudności" color="#a78bfa">
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            {DIFFICULTY.map(d => (
              <button key={d.id} onClick={() => setSelDifficulty(d)} className="p-3 rounded-xl text-left transition-all hover:scale-[1.02]"
                style={{ background: selDifficulty?.id === d.id ? 'rgba(167,139,250,0.15)' : 'rgba(255,255,255,0.03)', border: `1px solid ${selDifficulty?.id === d.id ? 'rgba(167,139,250,0.5)' : 'rgba(255,255,255,0.08)'}` }}>
        ...
        <div className="text-center space-y-2">
          <p className="text-[10px] text-white/30">Każde kliknięcie generuje unikalną losową scenę 🎲</p>
          <button onClick={handleGenerate} disabled={!canGen} className="inline-flex items-center gap-3 px-10 py-4 rounded-2xl font-black text-sm uppercase tracking-[0.2em] transition-all"
        ...
        {prompt && (
          <div ref={promptRef}>
            <PromptOutput prompt={prompt} onRegenerate={handleGenerate} color="#a78bfa" promptId="coloring-prompt" />
            <div className="mt-4 rounded-xl p-4 text-[10px] text-white/40" style={{ background: 'rgba(167,139,250,0.05)', border: '1px solid rgba(167,139,250,0.1)' }}>
              💡 <strong style={{ color: '#a78bfa' }}>KDP tip:</strong> Minimum 24 strony. Każda ilustracja + pusta strona za nią = 2 strony. 30 ilustracji = 60 stron. Rozmiar: 8,5 x 11 cali, 300 DPI.
            </div>
          </div>
        )}
      </div>
...
const MERCH_CATEGORIES = [
  ...
  { id: 'nature',      emoji: '🌿', label: 'Natura / Boho',    prompts: ['a sun and moon face with botanical frame of wildflowers','a mountain landscape with pine trees in geometric triangle','a wolf silhouette howling at moon in forest of geometric trees','a deer in enchanted forest with floral mandala frame','a hummingbird with tropical flowers in boho style','a bear paw with mountain and pine tree inside','crystals and wildflowers in a circular mandala','a whale swimming through cosmic starfield with moon'] },
  ...
  { id: 'funny',       emoji: '😂', label: 'Humor / Memy',     prompts: ['a grumpy cat face with I hate Mondays space for text','a confused dog face with Why? expression comic style','a pizza slice wearing sunglasses on beach','tacos doing karate with flames and action lines','a coffee cup with superhero cape saving the morning','a sloth hanging from branch with motivational quote space','a dinosaur doing yoga poses on yoga mat','a bear holding a giant fish trophy'] },
...
function MerchCreatorView({ onBack }) {
  const [selCategory, setSelCategory] = useState(null);
  const [selStyle, setSelStyle] = useState(null);
  ...
  function buildPrompt() {
    ...
    return `T-shirt graphic design of ${scene}${textPart}. ${selStyle.prompt}. IMPORTANT: transparent background, isolated graphic, no background, print-ready for direct garment printing (DTG), high contrast, works on both light and dark fabric, vector-style clean edges. Professional merchandise design, Amazon Merch on Demand ready. --ar 1:1 --style raw --v 6.1`;
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pb-24">
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-[10px] text-white/25 mt-2">✅ Przezroczyste tło &nbsp;·&nbsp; ✅ Amazon Merch ready &nbsp;·&nbsp; ✅ Tekst PL / EN / DE</p>
      </div>
      <div className="space-y-10">
        <Step n="1" title="Wybierz kategorię" color="#22d3ee">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {MERCH_CATEGORIES.map(cat => (
              <button key={cat.id} onClick={() => setSelCategory(cat)} className="relative p-4 rounded-xl text-left transition-all hover:scale-[1.02]"
                style={{ background: selCategory?.id === cat.id ? 'rgba(34,211,238,0.1)' : 'rgba(255,255,255,0.03)', border: `1px solid ${selCategory?.id === cat.id ? 'rgba(34,211,238,0.4)' : 'rgba(255,255,255,0.08)'}` }}>
                <div className="text-2xl mb-1">{cat.emoji}</div>
                <div className="text-xs font-black uppercase text-white">{cat.label}</div>
                <div className="text-[9px] text-white/30 mt-0.5">{cat.prompts.length} grafik</div>
                {selCategory?.id === cat.id && <div className="absolute top-2 right-2 w-4 h-4 rounded-full flex items-center justify-center" style={{ background: '#22d3ee' }}><Check className="w-2.5 h-2.5 text-black" /></div>}
              </button>
            ))}
          </div>
        </Step>
        <Step n="2" title="Wybierz styl" color="#22d3ee">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {MERCH_STYLES.map(s => (
              <button key={s.id} onClick={() => setSelStyle(s)} className="relative p-4 rounded-xl text-left transition-all hover:scale-[1.02]"
        ...
        <div className="text-center space-y-2">
          <p className="text-[10px] text-white/30">Każde kliknięcie generuje unikalny losowy design 🎲</p>
          <button onClick={handleGenerate} disabled={!canGen} className="inline-flex items-center gap-3 px-10 py-4 rounded-2xl font-black text-sm uppercase tracking-[0.2em] transition-all"
        ...
        {prompt && (
          <div ref={promptRef}>
            <PromptOutput prompt={prompt} onRegenerate={handleGenerate} color="#22d3ee" promptId="merch-prompt" />
            <div className="mt-4 rounded-xl p-4 text-[10px] text-white/40" style={{ background: 'rgba(34,211,238,0.05)', border: '1px solid rgba(34,211,238,0.1)' }}>
              💡 <strong style={{ color: '#22d3ee' }}>Merch tip:</strong> Po wygenerowaniu usuń tło w remove.bg lub Adobe Express. Wgraj PNG na Amazon Merch on Demand, Redbubble lub Printful. Zalecane: 4500 x 5400px, 300 DPI.
            </div>
          </div>
        )}
      </div>
...
// =========================================================================
// MENU GŁÓWNE — APLIKACJE 2
// =========================================================================
const TOOLS = [
  { id: 'text',     emoji: '✍️', label: 'Kreator Napisów',    subtitle: 'Generator Promptów Napisów AI',    desc: 'Artystyczne litery i napisy — kwiaty, ogień, lód, złoto i więcej.',        color: '#f59e0b', badge: 'TEXT STUDIO',     available: true },
  { id: 'coloring', emoji: '🎨', label: 'Kreator Kolorowanek', subtitle: 'Generator Kolorowanek AI',         desc: 'Prompty kolorowanek dla dzieci i dorosłych — gotowe na Amazon KDP.',        color: '#a78bfa', badge: 'COLORING STUDIO', available: true },
  { id: 'merch',    emoji: '👕', label: 'Kreator Koszulek',   subtitle: 'Generator Grafik na Koszulki AI',  desc: 'Grafiki na koszulki i bluzy — Amazon Merch, Redbubble, Printful.',           color: '#22d3ee', badge: 'MERCH STUDIO',    available: true },
  { id: null,       emoji: '🖼️', label: 'Kreator Okładek',    subtitle: 'Generator Okładek AI',             desc: 'Profesjonalne okładki książek i albumów. Wkrótce dostępne.',                color: '#f472b6', badge: 'WKRÓTCE',         available: false },
];

function MainMenu({ onSelect }) {
  return (
    <div className="min-h-screen bg-black px-3 sm:px-4 py-6 sm:py-12 font-sans">
      <style>{`
        @keyframes float3d2 {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-3xl mx-auto">
          {TOOLS.map((tool, i) => (
            <button key={i}
              onClick={() => tool.available && tool.id && onSelect(tool.id)}
              className={`${tool.available ? 'card3d2' : 'card3d2-disabled'} relative rounded-3xl p-8 border text-left group ${tool.available ? 'cursor-pointer' : 'cursor-default'}`}
              style={{
                background: `linear-gradient(135deg, ${tool.color}20, ${tool.color}10)`,
                borderColor: tool.available ? `${tool.color}44` : 'rgba(255,255,255,0.06)',
                boxShadow: tool.available ? `0 20px 60px ${tool.color}30, 0 4px 20px rgba(0,0,0,0.3)` : '0 4px 20px rgba(0,0,0,0.15)',
              }}>
              {/* Badge */}
              <div className="absolute top-4 right-4 text-[8px] font-black uppercase tracking-[0.2em] px-2 py-1 rounded-full"
                ...
                {tool.available ? tool.badge : 'WKRÓTCE'}
              </div>
              {/* Icon */}
              <div className="mb-6 text-6xl" style={{ filter: tool.available ? `drop-shadow(0 4px 12px ${tool.color}40)` : 'none' }}>
                {tool.emoji}
              </div>
              {/* Content */}
              ...
              {tool.available && (
                <div className="absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-400 pointer-events-none"
                  style={{ background: `radial-gradient(circle at 50% 0%, ${tool.color}25 0%, transparent 70%)` }} />
              )}
            </button>
          ))}
        </div>
...
export default function App2() {
  const [view, setView] = useState('menu'); // 'menu' | 'text' | 'coloring' | 'merch'

  return (
    <div className="min-h-screen bg-black text-white" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      <Nav />
      {view === 'menu'     && <MainMenu onSelect={setView} />}
      {view === 'text'     && <TextCreatorView onBack={() => setView('menu')} />}
      {view === 'coloring' && <ColoringCreatorView onBack={() => setView('menu')} />}
      {view === 'merch'    && <MerchCreatorView onBack={() => setView('menu')} />}
    </div>
  );
}
